...
def get_avg_color(img):
	...
	if type(pixels) is not int:
		...
		if count > 0:
			...
			raise IOError("讀取圖片失敗")
	else:
		raise IOError("PIL 讀取圖片失敗")
...
def find_closiest(color, list_colors):
	...
	if not cur_closer:
		raise ValueError("沒有足夠近似的照片，建議提高重複率")
	cur_closer[3] += 1
	...
	for y1 in range(0, height, SLICE_SIZE):
		for x1 in range(0, width, SLICE_SIZE):
			try:
				...
				if color[2]<=0.05:
					back_name='(0.0, 0.0, 0.0).jpg'
					paste_img = Image.open(OUT_DIR+back_name)
				else:
...
def get_image_paths():
	...
	for file_ in os.listdir(IN_DIR):
		...
		if suffix in suffixs:
			...
			print("非照片:%s" % file_)
	if len(paths) > 0:
		print("一共找到了%s" % len(paths) + "張照片")
	else:
		raise IOError("未找到任何照片")
